...
def question2_4() -> str:
    ...
    """
    answer = "In Q-learning update, we only take gamma to the power 1 into account, meanwhile in the Monte Carlo update, the power of the discount rate varies from 1 to the length of the episode. If we change the learning rate gamma, the relative difference (gamma**k-gamma’**k)/gamma**k between the old and the new gamma to the power k increases with the power k we take into account. Consequently the impact of the change of gamma on the Monte Carlo update will be significantly larger then the impact on the Q-learning update."  # TYPE YOUR ANSWER HERE (100 words max)
    return answer
...
def question3_6() -> str:
    ...
    return: answer (str): your answer as a string (100 words max)
    """
    answer = "Using the exploration fraction method, we know that for the specified amount of training time the proportion of exploration and exploitation changes steadily. While using the rate parameter, we don’t have that much control over the proportion of time during which we will continue to update epsilon, and consequently we may unintentionally set a too high value of the decay, which will result in too little exploration, or too low, which will cause too little exploitation.  Using the exploration fraction we will automatically adjust to the specific environment in terms of the proportion of time spent updating epsilon."  # TYPE YOUR ANSWER HERE (100 words max)
    return answer
...
def question3_7() -> str:
    ...
    """
    answer = "The loss function is not behaving as in typical supervised learning approaches because as the agent learns, predicting the exact value of the reward plus the target value of gets more difficult. At the beginning, the rewards the agents receives are quite steady – be it 0 or -1 per transition – and the target values are closer to their initialized values, making estimation relatively simple. However, as the agent learns, the rewards are getting higher and so does the values of the next actions– thus their estimation is getting more and more difficult. In DQN we also face the difficulty of the moving target, and its presence is more observable when the values of the next actions are getting higher, that is in the later stage of learning, and precise estimation gets harder."  # TYPE YOUR ANSWER HERE (150 words max)
    return answer
...
def question3_8() -> str:
    ...
    return: answer (str): your answer as a string (100 words max)
    """
    answer = "The spikes correspond to the updates to the target network. These are the moments when the target “moves” so our estimation is less precise"  # TYPE YOUR ANSWER HERE (100 words max)
    return answer
