...
NUM_SEEDS_SWEEP = 12 # NUMBER OF SEEDS TO USE FOR EACH HYPERPARAMETER CONFIGURATION
...
def train(env: gym.Env, config, output: bool = True) -> Tuple[np.ndarray, np.ndarray, np.ndarray, Dict]:
    ...
    with tqdm(total=config["max_timesteps"]) as pbar:
        while timesteps_elapsed < config["max_timesteps"]:
            ...
            for k, v in ep_data.items():
                run_data[k].extend(v)
                #print("k v in ep data items=",run_data[k])
    ...
    if config["plot_loss"]:
        ...
        #print(run_data["train_timesteps"],"\n\n\n jejujej!\n\n\n", run_data["q_loss"])
